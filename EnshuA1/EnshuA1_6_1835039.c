...
#define ITERATION 10000 // Number of Iteration
#define ETA 0.5// Learning coefficeint
...
int load_data(float x[][INPUT_DIM], float y[][OUTPUT_DIM]) {
    ...
    if ((ifp = fopen("kadai_data_mlp1.dat", "r")) == NULL) {
...
float act_func(float a){
    // return a;
    //反復回数3000付近で損失関数打ち止め〜3.179

// if(a<=0) return 0;
// else return a;
// #define ITERATION 6000 // Number of Iteration
// #define ETA 0.02 // Learning coefficeint

    return a * (1.0 / (1+exp(-a)));
    //#define ITERATION 8100 // Number of Iteration
//#define ETA 0.15 // Learning coefficeint
}
...
// return 1;
 ...
 return ((1-act_func(a)/a)*act_func(a)/a);
// return exp(a)*a*(2*(exp(a)+1)+a)/(exp(a)+1)*(exp(a)+1);
}
