...
int paranum; //関数の引数に変数の数を渡すためグローバルに用意

void pivot(double a[][paranum], double c[paranum][paranum], int p);
double atof_extension(char *txtdata, int input_num); //係数にKが含まれている場合の対応
void lntrim(char *str);                              //fgetによる改行文字を取り除く
double checkInverseMatrix(double A[][paranum]);      //逆行列が存在するなら1を返す
double calcDet(double A[][paranum]);                 //行列式を計算
int main(void)
{
    int i, j, k;
    int count;
...
double checkInverseMatrix(double A[][paranum])
{
...
double calcDet(double A[][paranum])
{

    double empty[paranum][paranum]; //ここではAについてだけピボット操作をしたいので、適当な配列を用意
